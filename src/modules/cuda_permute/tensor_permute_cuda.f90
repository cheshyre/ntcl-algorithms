module tensor_permute_cuda_module
    ...
    public :: tensor_permute_cuda

    type, extends(raw_tensor_permute) :: tensor_permute_cuda
    ...
    end type tensor_permute_cuda

    interface tensor_permute_cuda
        module procedure constructor_empty
        module procedure constructor
    end interface tensor_permute_cuda

    interface
        subroutine cuda_tensor_permute_generic_real32_execute(d_src, d_dst, number_of_elements, d_sdims, d_perm, order) &
                    bind(C, name="cuda_tensor_permute_generic_real32_execute")
            import :: c_ptr
            import :: c_int
            import :: c_int64_t
        ...
        end subroutine cuda_tensor_permute_generic_real32_execute

        subroutine cuda_tensor_permute_generic_real64_execute(d_src, d_dst, number_of_elements, d_sdims, d_perm, order) &
                    bind(C, name="cuda_tensor_permute_generic_real64_execute")
            import :: c_ptr
            import :: c_int
            import :: c_int64_t
            ...
            type(c_ptr), value, intent(in) :: d_sdims, d_perm
            integer(c_int), value, intent(in) :: order
        end subroutine cuda_tensor_permute_generic_real64_execute

        subroutine cuda_tensor_permute_generic_complex64_execute(d_src, d_dst, number_of_elements, d_sdims, d_perm, order) &
                    bind(C, name="cuda_tensor_permute_generic_complex64_execute")
            import :: c_ptr
            import :: c_int
            import :: c_int64_t
            ...
            type(c_ptr), value, intent(in) :: d_sdims, d_perm
            integer(c_int), value, intent(in) :: order
        end subroutine cuda_tensor_permute_generic_complex64_execute

        subroutine cuda_tensor_permute_generic_complex128_execute(d_src, d_dst, number_of_elements, d_sdims, d_perm, order) &
                    bind(C, name="cuda_tensor_permute_generic_complex128_execute")
            import :: c_ptr
            import :: c_int
            import :: c_int64_t
            ...
            type(c_ptr), value, intent(in) :: d_sdims, d_perm
            integer(c_int), value, intent(in) :: order
        end subroutine cuda_tensor_permute_generic_complex128_execute
    end interface

    integer, parameter :: max_tensor_order = 6
contains
    function constructor_empty() result(this)
        type(tensor_permute_cuda) :: this
    ...
    subroutine set_converters(this, c_converter, f_converter)
        class(tensor_permute_cuda), intent(inout) :: this
        type(tensor_c_pointer_converter), intent(in) :: c_converter
        type(tensor_fortran_converter), intent(in) :: f_converter
    ...
    subroutine permute_tensor(this, src, dst, perm, astream)
        ...
        if ( .not. this%is_compatible(src, dst, perm) ) &
                error stop "tensor_permute_cuda::permute_tensor:Tensors are not compatible."
        ...
        case ( dt_r32 )
            call cuda_tensor_permute_generic_real32_execute( &
                    src_device_cptr, dst_device_cptr, src%get_number_of_elements(), &
                    this%device_dims_cptr, this%device_perms_cptr, src%get_rank())
        case ( dt_r64 )
            call cuda_tensor_permute_generic_real64_execute( &
                    src_device_cptr, dst_device_cptr, src%get_number_of_elements(), &
                    this%device_dims_cptr, this%device_perms_cptr, src%get_rank())
        case ( dt_c64 )
            call cuda_tensor_permute_generic_complex64_execute( &
                    src_device_cptr, dst_device_cptr, src%get_number_of_elements(), &
                    this%device_dims_cptr, this%device_perms_cptr, src%get_rank())
        case ( dt_c128 )
            call cuda_tensor_permute_generic_complex128_execute( &
                    src_device_cptr, dst_device_cptr, src%get_number_of_elements(), &
                    this%device_dims_cptr, this%device_perms_cptr, src%get_rank())
        case default
            error stop "tensor_permute_cuda::permute_tensor:Datatype not supported."
        end select
        call this%synchronize(astream)
    ...
    subroutine synchronize(this, astream)
        class(tensor_permute_cuda), intent(in) :: this
        type(stream), intent(in), optional :: astream
        ...
        if (error /= 0) &
                error stop "tensor_permute_cuda::synchronize:Error in call to synchronize wrapper."
...
end module tensor_permute_cuda_module
