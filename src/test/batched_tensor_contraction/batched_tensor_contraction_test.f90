module batched_tensor_contraction_test_module
    use :: util_api, only : &
            assert, &
            string, &
            dictionary
    ...
    use :: batched_tensor_contraction_test_helper_module, only : &
            batched_tensor_contraction_test_helper

    implicit none
    private

    public :: batched_tensor_contraction_test

    type :: batched_tensor_contraction_test
    contains
        procedure :: run => run
        procedure :: cleanup => cleanup
        procedure :: clear => clear
    end type batched_tensor_contraction_test

    interface batched_tensor_contraction_test
        module procedure constructor
    end interface batched_tensor_contraction_test
contains
    function constructor() result(this)
        type(batched_tensor_contraction_test) :: this

        call this%clear()
    end function constructor

    subroutine run(this, assertion)
        class(batched_tensor_contraction_test), intent(in) :: this
        type(assert), intent(inout) :: assertion

        type(string), dimension(:), allocatable :: drivers, memory_types
        integer :: idx, midx
        type(string) :: prefix, my_prefix
        type(batched_tensor_contraction_test_helper) :: helper
        type(dictionary) :: options
        type(string), dimension(1) :: priorities

        call assertion%equal("batched_tensor_contraction::Test complete", .true.)

        drivers = batched_tensor_contraction_factory%get_available_btc_drivers()
        memory_types = memory_factory%get_available_memory_types()

        prefix = "batched_tensor_contraction::"
        ...
        do idx = 1, size(drivers)
            do midx = 1, size(memory_types)
                my_prefix = prefix%char_array//"memory_type="//memory_types(midx)%char_array//","// &
                        "btc_driver="//drivers(idx)%char_array//":"
        ...
        do idx = 1, size(drivers)
            do midx = 1, size(memory_types)
                ...
                call options%set_value("btc_driver", drivers(idx)%char_array)
                my_prefix = prefix%char_array//"options(memory_type)="//memory_types(midx)%char_array//","// &
                        "options(btc_driver)="//drivers(idx)%char_array//":"
    ...
    subroutine cleanup(this)
        class(batched_tensor_contraction_test), intent(inout) :: this
...
end module batched_tensor_contraction_test_module
